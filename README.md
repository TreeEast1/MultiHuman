...
把上面预测出的 NASA，加上这次任务真实的步骤完成情况，合成预测版 S。步骤不另做预测。试了两种权重，都是同一套 84 条预测：

| 权重 | 预测 S 范围 | 均值 | 和真实 S 对上的程度 |
|---|---|---:|---|
| 步骤 0.4 + NASA 0.6（现在代码） | 0.25–0.88 | 0.57 | R² **0.84** |
| 步骤 0.5 + NASA 0.5（对半，更稳妥） | 0.23–0.90 | 0.59 | R² **0.91** |

两种算法下，预测最高都是被试 22 的任务 3，最低都是被试 26 的任务 5_6。对半掺时 S 更听步骤的，数字稍高一点、和真实 S 也对得更紧——这是权重把 NASA 预测误差缩小了，不是又换了一个更强的模型。改权重 **不会** 改变上面的 NASA R² / F1。

明细：`工作目录/03_建模/s_score_from_nasa84/output_from_xgb_nasa/s_from_xgb_nasa.csv`（表里是 0.4 / 0.6 这一版）。
