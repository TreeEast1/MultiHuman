...
## 📅 2026-08-30：趋势预测与预警整套（双模块 + 软件静态案例）

总览：[趋势预警_总览.md](工作目录/03_建模/forecast_next_stage/趋势预警_总览.md)

软件「趋势预测与预警」做成双模块：**主图**画窗级人因（左已观察，右为 Transformer 瞬时虚线 + Ridge 整体走势）；**右侧**只写预测 S 做正常 / 预警。S 不直接当回归目标，也不从虚线反算。

- 正式预警：已观察 27 → Ridge 补整场 27 → 冻结 XGB → 公式 S。验证组 5 人、17 条，**S R² = 0.948，MAE = 0.025**。
- 静态示范：被试 2 · 任务 5_6，预测 S = **0.679**，阈值 0.51 → **正常**。接入包：`forecast_next_stage/case_subject_02_task_5_6/`。
- 窗级 Transformer 拼回 264 再算 S、或双头直接出 27 维，均不采用（NASA 为负或塌成常数）。

正式 Word：[趋势预测_跨被试未来阶段人因与S_实验报告.docx](工作目录/03_建模/forecast_next_stage/趋势预测_跨被试未来阶段人因与S_实验报告.docx)
