...
| **步骤 0.6 + NASA 0.4（现在用这个）** | **0.22–0.92** | **0.60** | **R² 0.95** |

对半掺是 0.91。要把 S 的 R² 提到 0.95，步骤得占六成、NASA 占四成。步骤本身是真实对错、模型没去猜，所以步骤权重大，S 自然更贴真值。**NASA 那条 R² 0.52 不会变。**

明细：`工作目录/03_建模/s_score_from_nasa84/output_from_xgb_nasa/s_from_xgb_nasa.csv`（`S_xgb` 是 0.6/0.4；表里还有 0.4/0.6、0.5/0.5 两列）。
