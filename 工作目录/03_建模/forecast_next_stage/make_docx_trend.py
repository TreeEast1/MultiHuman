...
def build_long():
    ...
    add_body(
        doc,
        "技术路线为：四模态连续信号按 30 秒窗、5 秒步切分，提取 66 个窗口指标；把一次任务按时间切成已观察阶段与未来阶段，只对已观察窗口汇总为均值、标准差、中位数与斜率，得到已观察段的 264 维；每一折仅在训练被试上按模态定额做互信息筛选，得到 27 维；用标准化 Ridge 把已观察 27 列映射为整场任务的同一 27 列；再用与正式完整观测口径相同的浅树 XGBoost 估计负荷；最后与真实步骤分合成 S。划分方式为按被试的 GroupKFold，同一人不同时出现在训练与验证。如图 1 所示。",
    )
    add_figure(doc, FIG / "fig1_pipeline.png", "图 1  趋势预测的技术路线", 15.0)
    add_body(
        doc,
        "与正式完整观测口径的差别在于：考试的人还没有整场人因，27 维由“已观察阶段＋Ridge”补出。下游 XGBoost、定额规则与合成公式保持不变。",
    )
    ...
    add_body(
        doc,
        "在 5 名新被试、17 条任务上，趋势预报 27 维后再经冻结 XGBoost 估计负荷分量，按正式公式合成绩效 S。合成 S 的 R² 为 0.948，MAE 为 0.025。验证组真值 S 范围为 0.42—0.86，均值 0.59。图 2 给出 17 条真值与预测的散点，点沿对角线聚集。",
    )
    ...
    add_figure(doc, FIG / "fig2_scatter.png", "图 2  验证组真值 S 与预测 S（17 条）", 11.5)
